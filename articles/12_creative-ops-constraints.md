...
まず、見落としがないようにできるだけ網羅的に洗い出した上で、ファクトに基づいて優先度をつける。前段で書いたとおり、材料にしたのは架電など別チャネルでの顧客の反応、営業への直接ヒアリング、類似業態の勝ちパターン、競合の表現です。

この作業が、早期の成果に繋がりました。

訴求軸の観点ではそれまで見落としていたものが当たり、表現の観点では他社で長期配信されていたものを起点に制作したものが効果を出しました。

この「一次情報を材料にする」という発想は、[#2 AIエージェントで広告運用を自動化して見えた「暗黙知」の壁](https://note.com/rapid_yeti215/n/n912be35cd5f6)で書いた、AIが平均点を超えるには現場の暗黙知が要る、という話とも関連します。
...
![検証設計の3つの打ち手を1枚にまとめた図。上部に「検証に使えるリソースは有限」という消えない制約を置き、①効果の大きい変数から週1テーマで「訴求→フォーマット→表現」と順に検証する時間軸を中央に、②各週に載せる候補を一次情報（顧客の反応・営業ヒアリング・類似業態の勝ちパターン・競合の表現）から作る流れを左に、③週の切り替わりで「間」をなくすための事前準備と4日での撤退の循環を下に配置し、帰結として月次CPAが翌月に3割以上改善したことを示す](../assets/article-images/12_creative-ops-constraints/img_01.png)

## AIで生産性を高めながら、制約を乗り越える設計が要る

検証に使えるリソースの有限性は、材料をいくら速く作っても解けない問題です。そして成果が出たのは、それに対して**設計**をしたからでした。効果の大きい順に週1テーマで検証し、各テーマの候補を一次情報から作り、どの結果に転んでも次をすぐ入れられるよう準備しておく。これが、AIで生産性が高まった先にある、もう一つの重要な要素でした。
